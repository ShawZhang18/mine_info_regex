...
#初犯偶犯
初犯偶犯 = r'本院认为.*(初犯|偶犯|无[\u4e00-\u9fa5]{0,10}前科).*判决如下'

#累犯或者有前科,两个结合判断,to do 有***前科中间会有标点符号？
累犯 = r'本院认为.*(曾犯|有前科|有[\u4e00-\u9fa5]{0,10}前科|累犯).*判决如下'

#邻里关系和民间矛盾
邻里关系 = r'本院认为.*(琐事|争执|互相推搡|纠纷|口角|争吵|对峙|冲突|厮打|互相推搡|相互殴打|互殴|相互辱骂).*判决如下'

#持械
持械 = r'本院认为.*(棍|刀|器具|匕首|凶器).*判决如下'

#手段残忍
残忍 = r'本院认为.*(残忍|情节恶劣).*判决如下'

# 自首
自首_不认定 = r'本院认为.*自首.{0,20}(不予|不符).*判决如下'
自首 = r'本院认为.*自首.*判决如下'

# 坦白
坦白_不认定 = r'本院认为.*(坦白|如实供述).{0,20}(不予|不符).*判决如下'
坦白 = r'本院认为.*(坦白|如实供述).*判决如下'

# 认罪
认罪悔罪_不认定 = r'本院认为.*(认罪|悔罪).{0,20}(不予|不符).*判决如下'
认罪悔罪 = r'本院认为.*(认罪|悔罪).*判决如下'

#正当防卫
正当防卫_防卫过当_不认定 = r'本院认为.*(正当防卫|防卫过当).{0,20}(不予|不符).*判决如下'
正当防卫_防卫过当 = r'本院认为.*(正当防卫|防卫过当).*判决如下'

#被害人过错
被害人过错_不认定 = r'本院认为.*(原告|被害人)[\u4e00-\u9fa5]{0,8}(过错|责任).{0,20}(不予|不符).*判决如下'
被害人过错 = r'本院认为.*(原告|被害人)[\u4e00-\u9fa5]{0,8}(过错|责任).*判决如下'

#刑罚情况
有期徒刑_年_月 = r'本院认为.*判决如下.{0,200}判(处|决).{0,5}(有期徒刑|拘役)[\u4e00-\u9fa5]{0,4}?' \
           r'([0-9]*[一二三四五六七八九十]{1,2})年[\u4e00-\u9fa5]{0,2}?' \
           r'(([0-9]*[一二三四五六七八九十]{0,2})个?月)?.*如不服'
有期徒刑_only月 = r'本院认为.*判决如下.{0,200}判(处|决).{0,5}(有期徒刑|拘役).{0,5}?([0-9]*[一二三四五六七八九十]{0,3})[个]?月.*(如不服|终审判决)'
死刑 = r'本院认为.*判决如下.{0,200}死刑.*(如不服|终审判决)'
无期徒刑 = r'本院认为.*判决如下.{0,200}无期徒刑.*(如不服|终审判决)'

#缓刑
缓刑 = r'本院认为.*判决如下.{0,200}(缓刑|缓期).*(如不服|终审判决)'
